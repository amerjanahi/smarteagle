...
import { approveSignup, listAllSignups, rejectSignup } from "@/lib/approvals.functions";
...
export function ApprovalCenter() {
  ...
  const listSignups = useServerFn(listAllSignups);
  const approveUser = useServerFn(approveSignup);
  const rejectUser = useServerFn(rejectSignup);
  ...
  const [approvingSignup, setApprovingSignup] = useState<UnifiedRequest | null>(null);
  const [signupAccess, setSignupAccess] = useState<"resident" | "staff">("resident");

  const requests = useQuery({
    queryKey: ["unified-user-approvals"],
    queryFn: async () => {
      const [pendingVilla, approvedVilla, rejectedVilla, profileRequests, signups] = await Promise.all([
      ...
      const signupRows: UnifiedRequest[] = signups.map((request: any) => ({
        id: request.id,
        type: "portal_access",
        ...
        email: request.email ?? "",
        summary: `New account · requested ${request.requested_role === "staff" ? "Staff" : "Resident"} access`,
        details: [
          { label: "Requested access", value: request.requested_role === "staff" ? "Staff" : "Resident" },
          { label: "Phone", value: request.phone ?? "—" },
          { label: "Current roles", value: request.roles?.length ? request.roles.join(", ") : "None — disabled" },
        ],
        source: request,
        kind: "signup",
      }));

      return [...signupRows, ...villaRows, ...profileRows].sort(
    ...
    mutationFn: async (request: UnifiedRequest) => {
      if (request.kind === "signup") {
        await approveUser({
          data: {
            userId: request.id,
            role: signupAccess === "staff" ? "operations" : "resident",
            fullName: request.name,
          },
        });
      } else if (request.kind === "villa") {
  ...
  const reject = useMutation({
    mutationFn: async () => {
      if (!rejecting) return;
      if (rejecting.kind === "signup") {
        await rejectUser({ data: { userId: rejecting.id, notes: reason } });
      } else if (rejecting.kind === "villa") {
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t pt-3">
              ...
              {request.status === "pending" && <div className="flex gap-2">
                <Button size="sm" onClick={() => {
                  if (request.kind === "signup") {
                    setSignupAccess(request.source.requested_role === "staff" ? "staff" : "resident");
                    setApprovingSignup(request);
                  } else {
                    approve.mutate(request);
                  }
                }} disabled={approve.isPending || reject.isPending}><Check className="mr-1 h-4 w-4" />Approve &amp; apply</Button>
      ...
      <Dialog open={!!approvingSignup} onOpenChange={(open) => !open && setApprovingSignup(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Approve portal access</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div>
              <p className="font-medium">{approvingSignup?.name}</p>
              <p className="text-sm text-muted-foreground">{approvingSignup?.email}</p>
            </div>
            <div className="grid gap-1.5">
              <Label>Portal access type</Label>
              <Select value={signupAccess} onValueChange={(value) => setSignupAccess(value as "resident" | "staff")}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="resident">Resident portal</SelectItem>
                  <SelectItem value="staff">Staff portal</SelectItem>
                </SelectContent>
              </Select>
              <p className="text-xs text-muted-foreground">
                Resident access can be linked to a villa after approval. Staff access opens the staff portal without resident property permissions.
              </p>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setApprovingSignup(null)}>Cancel</Button>
            <Button onClick={() => approvingSignup && approve.mutate(approvingSignup)} disabled={approve.isPending}>
...
async function invalidateApprovals(queryClient: ReturnType<typeof useQueryClient>) {
  await Promise.all([
    ...
    queryClient.invalidateQueries({ queryKey: ["all-signups"] }),
  ]);
}
